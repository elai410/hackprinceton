1. Repository structure
  ...
  │
  └── docs/                        # (existing; do not modify during build)
      ├── AGENTS.md
      ├── build-spec.md
  ...
  ---                                                                                                                                                      
  3. Interface contracts (no implementation yet)
  ...
  - Returns one or more ValidationError if any SkillCall references an unknown skill_id, any argument is out of bounds, or steps is empty (unless flagged).
  ...
  ---             
  Contract 2 — adapters/base.py (WS-A defines Protocol; WS-B implements; WS-B/C consume)                                                                   
  ...
  - If the call fails at the hardware level, it returns a StepResult with status="failed" and a detail string — it does not raise.                         
  ...
  Exact request/response shapes from build-spec.md §JSON contracts. Nothing new here — but WS-D must treat this as the only stable interface to the
  companion. WS-D must not assume any additional fields.                                                                                                   
  ...
  TypeScript mirrors of Contract 0. WS-D writes these; WS-A signs off that field names match exactly. These are the only types WS-D uses — no any, no
  ad-hoc inline shapes in api.ts.                                                                                                                          
  ...
  Hour 0–1   [WS-A]  models.py + examples/manifest.adeept.json
                     ← GATE: everyone blocked on this; do it first, together if needed                                                                     
                                                                                                                                                           
  Hour 0–2   [WS-A]  validate.py + plan.valid.json + plan.invalid.json + plan.fallback.json                                                                
             [WS-D]  web/src/types.ts (mirrors models.py as it's written)                                                                                  
                     ← these two can parallel once models.py is 80% stable                                                                                 
                                                                                                                                                           
  Hour 1–3   [WS-B]  settings.py, adapters/base.py, adapters/mock.py                                                                                       
             [WS-B]  main.py (app skeleton, CORS, mount routers as stubs)                                                                                  
             [WS-B]  routes/health.py, routes/execute.py                                                                                                   
                     ← exit: curl GET /health and POST /execute with valid plan returns trace
                                                                                                                                                           
  Hour 2–4   [WS-D]  web/ scaffold: index.html, vite config, api.ts, state.ts
             [WS-D]  PlanForm.ts + BlockList.ts + main.ts wired to POST /execute with plan.valid.json                                                      
                     ← exit: browser can trigger execute, see block list, no planner yet                                                                   
                                                                                                                                                           
  Hour 3–6   [WS-C]  planner/prompts.py (system prompt template w/ manifest injection)                                                                     
  ...
  Hour 4–6   [WS-D]  ReasoningStream.ts, clarification reply UI                                                                                            
             [WS-D]  Wire POST /plan → show reasoning → confirm → POST /execute                                                                            
                     ← exit: full end-to-end in browser with mock adapter                                                                                  
   
  Hour 5–8   [WS-B]  adapters/adeept.py (real hardware; needs physical access)                                                                             
                     ← exit: same curl plan moves arm
                                                                                                                                                           
  Hour 7–10  [ALL]   Integration test: browser NL → K2 → plan → execute on real arm                                                                        
  ...
  Hour 10+   [ALL]   Demo script, rehearsal, hotspot test, backup video                                                                                    
